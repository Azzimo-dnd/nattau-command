...
export function MiniatureViewer() {
  ...
  const loadFile = async (file: File) => {
    ...
    if (!file.name.toLowerCase().endsWith(".stl")) {
      setError("For this prototype, choose an STL file.");
      return;
    }
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_320px]">
      <div className="overflow-hidden rounded-[30px] border border-slate-800 bg-[#080d13] shadow-2xl shadow-black/30">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-800 bg-slate-950/70 px-4 py-3 sm:px-5">
          ...
          <div className="flex flex-wrap gap-2">
            <button type="button" onClick={() => setAutoRotate((value) => !value)} className={`rounded-xl border px-3 py-2 text-xs font-bold ${autoRotate ? "border-cyan-400/50 bg-cyan-400/10 text-cyan-200" : "border-slate-700 text-slate-400"}`}>
        ...
        >
          {model ? (
            ...
            <button type="button" onClick={() => inputRef.current?.click()} className="absolute inset-0 flex w-full flex-col items-center justify-center gap-4 bg-[radial-gradient(circle_at_50%_38%,rgba(39,66,92,0.5),rgba(8,13,19,0.95)_56%)] px-8 text-center">
              ...
              <span className="text-2xl font-black text-slate-100">Drop Pippo.stl here</span>
              <span className="max-w-xl text-sm leading-6 text-slate-400">or click to choose an STL from your computer. The prototype reads the file locally in your browser; nothing is uploaded or saved yet.</span>
            </button>
          )}
          {model ? (
      ...
      <aside className="space-y-4">
        <div className="rounded-[26px] border border-slate-800 bg-slate-900/75 p-5">
          <p className="text-xs font-black uppercase tracking-[0.22em] text-yellow-400">Prototype controls</p>
          <h2 className="mt-2 text-xl font-black text-slate-100">Current miniature</h2>
          <p className="mt-2 text-xs leading-5 text-slate-500">This is deliberately separate from character profiles for the first pass. Once the viewing experience feels right, the same component can be embedded into the Characters tab.</p>

          <input ref={inputRef} type="file" accept=".stl,model/stl" className="hidden" onChange={(event) => { const file = event.target.files?.[0]; if (file) void loadFile(file); event.currentTarget.value = ""; }} />
        ...
        <div className="rounded-[26px] border border-slate-800 bg-slate-900/75 p-5">
          ...
          {model ? (
            ...
            <p className="mt-3 text-xs leading-5 text-slate-600">Load the sample STL to inspect its web weight and dimensions.</p>
          )}
        </div>
