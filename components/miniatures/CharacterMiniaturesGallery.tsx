...
export function CharacterMiniaturesGallery({ campaignId, currentUserId, isDm, preferredPlayerId = null }: Props) {
  ...
  const selected = useMemo(() => roster.find((row) => row.player_id === selectedId) ?? null, [roster, selectedId]);
  ...
  useEffect(() => {
    ...
    const loadRoster = async () => {
      ...
      try {
        ...
        setRoster(rows);
        const preferred = preferredPlayerId && rows.some((row) => row.player_id === preferredPlayerId) ? preferredPlayerId : null;
        ...
        <section className="rounded-[26px] border border-slate-800 bg-slate-900/55 p-4 sm:p-5">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p className="mt-2 text-sm text-slate-500">{selected.miniature_id ? `${selected.original_name} · ${formatBytes(selected.file_size_bytes)} STL · ${selected.triangle_count?.toLocaleString() ?? "?"} triangles` : "The Game Master has not assigned a miniature to this character yet."}</p>
              {selected.web_storage_path ? <p className="mt-1 text-xs font-bold text-emerald-300">Web GLB active · {formatBytes(selected.web_file_size_bytes)}</p> : selected.miniature_id ? <p className="mt-1 text-xs text-yellow-300">Using source STL until the GM generates a web GLB.</p> : null}
            </div>
            <div className="flex flex-wrap gap-2">{loadingModel || loadingSkin ? <span className="self-center text-xs font-semibold text-cyan-300">Loading private model…</span> : null}{canPaintSelected ? <Link href={paintHref} className="inline-flex min-h-10 items-center rounded-xl border border-fuchsia-400/30 bg-fuchsia-400/10 px-4 text-xs font-black text-fuchsia-100">{isDm ? "Paint miniature" : selected.player_id === currentUserId ? "Paint my miniature" : `Paint ${selected.display_name}`}</Link> : null}</div>
          </div>
        </section>

        {selected.miniature_id ? <section className="rounded-[26px] border border-slate-800 bg-slate-900/55 p-4 sm:p-5">
          <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.22em] text-cyan-300">Skins</p><p className="mt-1 text-xs text-slate-500">Everyone can preview skins. Players may contribute one replaceable skin to someone else&apos;s miniature; only the owner or GM chooses the default.</p></div><span className="rounded-full border border-slate-700 px-3 py-1 text-xs font-bold text-slate-500">{skins.length} saved</span></div>
          <div className="mt-4 flex gap-2 overflow-x-auto pb-2">
            <div className={`min-w-[210px] rounded-2xl border p-3 ${selectedSkinId === null ? "border-cyan-400/40 bg-cyan-400/5" : "border-slate-800 bg-black/10"}`}><div className="flex items-start justify-between gap-2"><div><p className="font-black text-slate-200">Original / unpainted</p><p className="mt-1 text-[11px] text-slate-600">Base miniature</p></div>{!hasDefault ? <span className="rounded-full bg-yellow-400/10 px-2 py-1 text-[9px] font-black uppercase text-yellow-300">Default</span> : null}</div><div className="mt-3 flex gap-2"><button type="button" disabled={loadingSkin} onClick={() => void chooseSkin(null)} className="rounded-lg border border-cyan-400/25 px-3 py-1.5 text-[11px] font-bold text-cyan-200">Preview</button>{canSetDefault && hasDefault ? <button type="button" disabled={busySkinId !== null} onClick={() => void setDefault(null)} className="rounded-lg border border-yellow-400/25 px-3 py-1.5 text-[11px] font-bold text-yellow-200">Set default</button> : null}</div></div>
            {skins.map((skin) => <div key={skin.id} className={`min-w-[230px] rounded-2xl border p-3 ${selectedSkinId === skin.id ? "border-fuchsia-400/40 bg-fuchsia-400/5" : "border-slate-800 bg-black/10"}`}><div className="flex items-start justify-between gap-2"><div className="min-w-0"><p className="truncate font-black text-slate-200">{skin.name}</p><p className="mt-1 truncate text-[11px] text-slate-600">by {skin.creator_display_name}</p>{skin.is_mine && skin.is_guest_contribution ? <p className="mt-1 text-[9px] font-black uppercase tracking-[0.12em] text-cyan-300">Your community skin</p> : null}</div>{skin.is_default ? <span className="rounded-full bg-yellow-400/10 px-2 py-1 text-[9px] font-black uppercase text-yellow-300">Default</span> : null}</div><div className="mt-3 flex gap-2"><button type="button" disabled={loadingSkin} onClick={() => void chooseSkin(skin)} className="rounded-lg border border-fuchsia-400/25 px-3 py-1.5 text-[11px] font-bold text-fuchsia-200">{busySkinId === skin.id && loadingSkin ? "Loading…" : "Preview"}</button>{canSetDefault && !skin.is_default ? <button type="button" disabled={busySkinId !== null} onClick={() => void setDefault(skin)} className="rounded-lg border border-yellow-400/25 px-3 py-1.5 text-[11px] font-bold text-yellow-200">Set default</button> : null}</div></div>)}
          </div>
        </section> : null}
