...
type PaletteCategory = "Primer" | "Skin" | "Cloth" | "Leather" | "Metal" | "Natural" | "Magic" | "Custom";
...
type PaletteEntry = { name: string; color: string; category: PaletteCategory };
type FocusRequest = { id: number; point: [number, number, number] } | null;

const BASE_PALETTE: PaletteEntry[] = [
...
function PaintMesh({
  ...
  const restore = useCallback(() => {
    ...
    for (const triangle of previewRef.current) {
      ...
      attr.setXYZ(offset, color.r, color.g, color.b);
      attr.setXYZ(offset + 1, color.r, color.g, color.b);
      attr.setXYZ(offset + 2, color.r, color.g, color.b);
    }
    ...
    while (head < tail) {
      ...
      for (let slot = 0; slot < 3; slot += 1) {
        ...
        if (activeTool === "smart") {
          ...
          const dot =
            topology.normals[offset] * topology.normals[neighborOffset]
  ...
  const regionFromEvent = useCallback((event: ThreeEvent<MouseEvent | PointerEvent>, activeTool: Tool) => {
    ...
    return collect(face, localPoint, activeTool);
  }, [collect]);
  ...
  const preview = useCallback((indices: number[]) => {
    ...
    for (const triangle of indices) {
      const offset = triangle * 3;
      attr.setXYZ(offset, highlight.r, highlight.g, highlight.b);
      attr.setXYZ(offset + 1, highlight.r, highlight.g, highlight.b);
      attr.setXYZ(offset + 2, highlight.r, highlight.g, highlight.b);
    }
    attr.needsUpdate = true;
    ...
    <>
      <mesh
        ...
        onPointerMove={(event) => {
          ...
          if (!paintMode || navigationActive || tool === "picker") {
            if (previewRef.current.length) restore();
            return;
          }
...
export function MiniaturePainterPrototypeV1({ sourceFile }: Props) {
  ...
  const [palette, setPalette] = useState<PaletteEntry[]>(() => [...BASE_PALETTE]);
    ...
    const load = async () => {
      ...
      try {
        ...
        for (let i = 0; i < colorAttribute.count; i += 1) {
          colorAttribute.setXYZ(i, primer.r, primer.g, primer.b);
        }
  ...
  const undoLast = () => {
    ...
    for (let i = 0; i < entry.indices.length; i += 1) {
      paintIds[entry.indices[i]] = entry.before[i];
    }
  ...
  const resetPaint = () => {
    ...
    for (let i = 0; i < attr.count; i += 1) {
      attr.setXYZ(i, primer.r, primer.g, primer.b);
    }
    attr.needsUpdate = true;
    setUndo([]);
  };
  ...
  if (!sourceFile) {
    return (
      <div className="rounded-[28px] border border-dashed border-slate-700 bg-slate-900/40 p-10 text-center text-slate-500">
        Choose a saved miniature above to start the painter experiment.
      </div>
    );
  }
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
      <div className="overflow-hidden rounded-[30px] border border-slate-800 bg-[#080d13]">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-800 px-4 py-3">
          <div>
            <p className="text-[10px] font-black uppercase tracking-[0.24em] text-fuchsia-300">Miniature Painter v0.2</p>
          ...
          <div className="flex flex-wrap gap-2">
            <button
              ...
              className={`rounded-xl border px-3 py-2 text-xs font-bold ${!paintMode ? "border-cyan-400/50 bg-cyan-400/10 text-cyan-200" : "border-slate-700 text-slate-400"}`}
            >
              View
            </button>
            <button
              ...
              disabled={!model}
              className={`rounded-xl border px-3 py-2 text-xs font-bold ${paintMode ? "border-fuchsia-400/50 bg-fuchsia-400/10 text-fuchsia-200" : "border-slate-700 text-slate-400"}`}
            >
              Paint
            </button>
            <button
        ...
        <div
          className={`relative h-[70vh] min-h-[580px] max-h-[900px] ${navigationActive ? "cursor-grabbing" : paintMode ? "cursor-crosshair" : "cursor-grab"}`}
        ...
        >
          {model && topology && paintIds ? (
            <Canvas shadows dpr={[1, 1.5]} camera={{ fov: 32 }}>
              ...
              <PaintMesh
                ...
                onHoverPoint={(point) => { hoverPointRef.current = point; }}
              />
            </Canvas>
          ) : (
          ...
          {model ? (
            <div className="pointer-events-none absolute bottom-4 left-1/2 max-w-[92%] -translate-x-1/2 rounded-2xl border border-slate-700 bg-slate-950/85 px-4 py-2 text-center text-[11px] leading-5 text-slate-400 backdrop-blur">
              {paintMode
                ? `${previewCount.toLocaleString()} triangles · LMB paint · RMB orbit · MMB pan · wheel zoom-to-cursor · Space+LMB orbit · Shift+Space+LMB pan · F focus · Alt+click pick · Ctrl+click Smart`
                : "LMB orbit · MMB zoom · RMB pan · wheel zoom-to-cursor · hover a detail and press F to make it the camera pivot"}
            </div>
          ) : null}
        </div>
      </div>
      ...
      <aside className="space-y-4">
        <div className="rounded-[26px] border border-slate-800 bg-slate-900/75 p-5">
          ...
          <div className="mt-3 grid grid-cols-2 gap-2">
            {([['smart', 'Smart'], ['brush', 'Brush'], ['triangle', 'Triangle'], ['shell', 'Shell'], ['picker', 'Picker']] as const).map(([id, label]) => (
              <button
                key={id}
                type="button"
                onClick={() => setTool(id)}
                className={`rounded-xl border px-3 py-2 text-xs font-bold ${tool === id ? "border-fuchsia-400/50 bg-fuchsia-400/10 text-fuchsia-100" : "border-slate-700 text-slate-400"}`}
              >
                {label}
              </button>
            ))}
          ...
          {tool === "smart" ? (
            <label className="mt-4 block text-xs text-slate-500">
              Edge threshold <b className="float-right text-slate-300">{angle}°</b>
              <input type="range" min="3" max="80" value={angle} onChange={(event) => setAngle(Number(event.target.value))} className="mt-2 w-full" />
            </label>
          ) : null}
          ...
          {tool === "brush" ? (
            <div className="mt-4 space-y-4">
              <div>
                <p className="text-xs font-semibold text-slate-500">Brush behaviour</p>
                <div className="mt-2 grid grid-cols-2 gap-2">
                  <button
                    type="button"
                    onClick={() => setBrushMode("surface")}
                    className={`rounded-xl border px-3 py-2 text-xs font-bold ${brushMode === "surface" ? "border-cyan-400/50 bg-cyan-400/10 text-cyan-100" : "border-slate-700 text-slate-400"}`}
                  >
                    Surface
                  </button>
                  <button
                    type="button"
                    onClick={() => setBrushMode("volume")}
                    className={`rounded-xl border px-3 py-2 text-xs font-bold ${brushMode === "volume" ? "border-cyan-400/50 bg-cyan-400/10 text-cyan-100" : "border-slate-700 text-slate-400"}`}
                  >
                    Volume
                  </button>
                </div>
                <p className="mt-2 text-[11px] leading-5 text-slate-600">
                  Surface respects sharp edges. Volume wraps through connected geometry inside the brush radius.
                </p>
              </div>
              <label className="block text-xs text-slate-500">
                Brush radius <b className="float-right text-slate-300">{radius.toFixed(1)} mm</b>
                <input type="range" min="0.5" max="12" step="0.5" value={radius} onChange={(event) => setRadius(Number(event.target.value))} className="mt-2 w-full" />
              </label>
            </div>
          ) : null}
        </div>
        ...
        <div className="rounded-[26px] border border-slate-800 bg-slate-900/75 p-5">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.2em] text-yellow-400">Colors</p>
              <p className="mt-1 text-[11px] text-slate-600">{palette.length} swatches</p>
            </div>
            <div className="h-9 w-9 rounded-xl border-2 border-cyan-300" style={{ background: palette[materialId]?.color ?? palette[0].color }} />
          </div>
          ...
          <div className="mt-3 flex flex-wrap gap-1.5">
            {CATEGORY_TABS.map((category) => (
              <button
                ...
                className={`rounded-lg border px-2 py-1 text-[10px] font-bold ${activeCategory === category ? "border-yellow-400/40 bg-yellow-400/10 text-yellow-100" : "border-slate-800 text-slate-500"}`}
              >
                {category}
              </button>
            ))}
          ...
          <div className="mt-3 max-h-64 overflow-y-auto pr-1">
            <div className="grid grid-cols-5 gap-2">
              {visiblePalette.map(({ entry, index }) => (
                <button
                  key={`${entry.name}-${index}`}
                  type="button"
                  title={entry.name}
                  aria-label={entry.name}
                  onClick={() => setMaterialId(index)}
                  className={`aspect-square rounded-xl border-2 ${materialId === index ? "border-cyan-300 ring-2 ring-cyan-300/20" : "border-slate-700"}`}
          ...
          <div className="mt-4 rounded-2xl border border-slate-800 bg-black/15 p-3">
            <p className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-500">Custom swatch</p>
            <div className="mt-2 flex gap-2">
              ...
              <input
                ...
                onChange={(event) => {
                  ...
                  setCustomColor(value.startsWith("#") ? value.slice(0, 7) : `#${value.slice(0, 6)}`);
                }}
                className="min-w-0 flex-1 rounded-lg border border-slate-700 bg-slate-950 px-3 text-xs font-bold uppercase text-slate-300 outline-none focus:border-cyan-400/50"
                maxLength={7}
              />
            </div>
            <button
              ...
              className="mt-2 min-h-9 w-full rounded-lg border border-cyan-400/30 bg-cyan-400/10 px-3 text-xs font-bold text-cyan-100"
            >
              Add custom color
            </button>
          </div>
        ...
        <div className="rounded-[26px] border border-slate-800 bg-slate-900/75 p-5">
          <p className="text-xs font-black uppercase tracking-[0.2em] text-cyan-300">Navigation while painting</p>
          <div className="mt-3 space-y-1.5 text-[11px] leading-5 text-slate-500">
            <p><b className="text-slate-300">RMB drag</b> orbit · <b className="text-slate-300">MMB drag</b> pan</p>
            <p><b className="text-slate-300">Wheel</b> zooms toward the cursor</p>
            <p><b className="text-slate-300">Space + LMB</b> temporary orbit</p>
            <p><b className="text-slate-300">Shift + Space + LMB</b> temporary pan</p>
            <p><b className="text-slate-300">F</b> pivots the camera around the hovered detail</p>
            <p><b className="text-slate-300">Alt + click</b> picker · <b className="text-slate-300">Ctrl + click</b> Smart</p>
          </div>
        </div>

        <div className="rounded-[26px] border border-slate-800 bg-slate-900/75 p-5">
          <div className="grid grid-cols-2 gap-2">
            <button type="button" onClick={undoLast} disabled={!undo.length} className="rounded-xl border border-slate-700 px-3 py-2 text-xs font-bold text-slate-300 disabled:opacity-30">Undo</button>
            <button type="button" onClick={resetPaint} disabled={!paintIds} className="rounded-xl border border-slate-700 px-3 py-2 text-xs font-bold text-slate-300 disabled:opacity-30">Reset paint</button>
          </div>
          <p className="mt-3 text-[11px] leading-5 text-slate-500">Painter v0.2 is still a temporary experiment. Paint is not saved to Supabase yet.</p>
        </div>
      </aside>
    </div>
  );
