...
export function MiniaturePainterPrototypeV1({ sourceFile }: Props) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const primer = new THREE.Color(BASE_PALETTE[0].color);
        for (let index = 0; index < colorAttribute.count; index += 1) {
          colorAttribute.setXYZ(index, primer.r, primer.g, primer.b);
        }
        colorAttribute.needsUpdate = true;
  ...
  const undoLast = () => {
    ...
    for (let index = 0; index < entry.indices.length; index += 1) {
      paintIds[entry.indices[index]] = entry.before[index];
    }
  ...
  if (!sourceFile) {
    return <div className="rounded-[28px] border border-dashed border-slate-700 bg-slate-900/40 p-10 text-center text-slate-500">Choose a saved miniature above to start the painter experiment.</div>;
  }
      ...
      <aside className="space-y-4">
        <div className="rounded-[26px] border border-slate-800 bg-slate-900/75 p-5">
          ...
          <div className="mt-3 grid grid-cols-2 gap-2">
            {([['smart', 'Smart'], ['brush', 'Brush'], ['triangle', 'Triangle'], ['shell', 'Shell'], ['picker', 'Picker']] as const).map(([id, label]) => (
              <button key={id} type="button" onClick={() => setTool(id)} className={`rounded-xl border px-3 py-2 text-xs font-bold ${tool === id ? "border-fuchsia-400/50 bg-fuchsia-400/10 text-fuchsia-100" : "border-slate-700 text-slate-400"}`}>{label}</button>
            ))}
          </div>
            ...
            <div className="mt-4 space-y-4">
              <div>
                ...
                <div className="mt-2 grid grid-cols-2 gap-2">
                  <button type="button" onClick={() => setBrushMode("surface")} className={`rounded-xl border px-3 py-2 text-xs font-bold ${brushMode === "surface" ? "border-cyan-400/50 bg-cyan-400/10 text-cyan-100" : "border-slate-700 text-slate-400"}`}>Surface</button>
                  <button type="button" onClick={() => setBrushMode("volume")} className={`rounded-xl border px-3 py-2 text-xs font-bold ${brushMode === "volume" ? "border-cyan-400/50 bg-cyan-400/10 text-cyan-100" : "border-slate-700 text-slate-400"}`}>Volume</button>
                </div>
                <p className="mt-2 text-[11px] leading-5 text-slate-600">Surface respects sharp edges. Volume wraps through connected geometry inside the brush radius.</p>
              </div>
        ...
        <div className="rounded-[26px] border border-slate-800 bg-slate-900/75 p-5">
          <div className="flex items-start justify-between gap-3">
            <div><p className="text-xs font-black uppercase tracking-[0.2em] text-yellow-400">Colors</p><p className="mt-1 text-[11px] text-slate-600">{palette.length} swatches</p></div>
          ...
          <div className="mt-3 flex flex-wrap gap-1.5">
            {CATEGORY_TABS.map((category) => (
              <button key={category} type="button" onClick={() => setActiveCategory(category)} className={`rounded-lg border px-2 py-1 text-[10px] font-bold ${activeCategory === category ? "border-yellow-400/40 bg-yellow-400/10 text-yellow-100" : "border-slate-800 text-slate-500"}`}>{category}</button>
            ))}
          </div>
          ...
          <div className="mt-3 max-h-64 overflow-y-auto pr-1">
            <div className="grid grid-cols-5 gap-2">
              {visiblePalette.map(({ entry, index }) => (
                <button key={`${entry.name}-${index}`} type="button" title={entry.name} aria-label={entry.name} onClick={() => setMaterialId(index)} className={`aspect-square rounded-xl border-2 ${materialId === index ? "border-cyan-300 ring-2 ring-cyan-300/20" : "border-slate-700"}`} style={{ background: entry.color }} />
              ))}
            </div>
          </div>
          ...
          <div className="mt-4 rounded-2xl border border-slate-800 bg-black/15 p-3">
            ...
            <div className="mt-2 flex items-center gap-3">
              <input type="color" value={customColor} onChange={(event) => setCustomColor(event.target.value)} className="h-10 w-14 cursor-pointer rounded-lg border border-slate-700 bg-transparent p-1" aria-label="Custom color" />
              <span className="text-xs font-bold uppercase text-slate-400">{customColor}</span>
            </div>
            <button type="button" onClick={addCustomSwatch} className="mt-2 min-h-9 w-full rounded-lg border border-cyan-400/30 bg-cyan-400/10 px-3 text-xs font-bold text-cyan-100">Add custom color</button>
          </div>
        </div>
        ...
        <div className="rounded-[26px] border border-slate-800 bg-slate-900/75 p-5">
          ...
          <div className="mt-3 space-y-1.5 text-[11px] leading-5 text-slate-500">
            ...
            <p><b className="text-slate-300">F</b> pivots around the hovered detail</p>
        ...
        <div className="rounded-[26px] border border-slate-800 bg-slate-900/75 p-5">
          ...
          <p className="mt-3 text-[11px] leading-5 text-slate-500">Painter v0.2 is still temporary. Paint is not saved to Supabase yet.</p>
        </div>
      </aside>
    </div>
  );
