...
import { PuzzleRune } from "../PuzzleRune";
...
export function RuneSequencePuzzle({ puzzle, run, disabled, onAction, onReveal }: {
  ...
  useEffect(() => () => { ++playbackVersion.current; playbackBusy.current = false; }, [run.id, run.status]);
  ...
  const reveal = async () => {
    if (disabled || playbackBusy.current) return;
    ...
    try {
      ...
      if (!sequence || version !== playbackVersion.current) return;
    ...
    <div className="mx-auto max-w-[680px] space-y-4">
      <div className="rounded-[30px] border border-violet-500/25 bg-[radial-gradient(circle_at_center,#2b1740_0%,#0d0713_68%)] p-6 text-center shadow-2xl shadow-violet-950/40">
        ...
        <div className="mt-5 flex min-h-32 items-center justify-center rounded-2xl border border-violet-400/15 bg-black/20">
          <span className={`text-7xl font-black transition ${lit ? "scale-110 text-violet-100 drop-shadow-[0_0_18px_rgba(196,181,253,0.9)]" : "text-violet-950"}`}><PuzzleRune rune={lit ?? "✦"} /></span>
        </div>
      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4">
        <div className="flex min-h-14 flex-wrap items-center justify-center gap-2 rounded-xl border border-slate-800 bg-slate-950/60 p-2">
          {Array.from({ length: needed }, (_, index) => <span key={index} className="flex h-10 w-10 items-center justify-center rounded-lg border border-violet-500/20 text-2xl text-violet-100"><PuzzleRune rune={input[index]} /></span>)}
        </div>
        <div className="mt-3 grid grid-cols-4 gap-2 sm:grid-cols-6 md:grid-cols-8">
          {runes.map((rune) => <button key={rune} type="button" disabled={disabled || showing || input.length >= needed} onClick={() => setInput((current) => [...current, rune])} className="min-h-11 rounded-xl border border-slate-700 bg-slate-950 text-2xl text-slate-100 disabled:opacity-35" aria-label={rune}><PuzzleRune rune={rune} label /></button>)}
        </div>
