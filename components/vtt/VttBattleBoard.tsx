...
import { VttSpotlightPanel } from "./VttSpotlightPanel";
...
type Props = { campaignSlug?: string; campaignId: string; isDm: boolean; currentUserId: string; currentUserName: string };
...
export function VttBattleBoard({ campaignId, campaignSlug = "nattau", isDm, currentUserId, currentUserName }: Props) {
  const daggerheart = campaignSlug === "barovia";
  const board = useVttBoard(campaignId, isDm, currentUserId);
  const dice = useVttDice({ daggerheart, campaignId, currentUserId, currentUserName, sceneId: board.scene?.id ?? null, sharedScene: Boolean(board.scene?.is_active && board.scene?.visible_to_players) });
  ...
  const fog = useVttFog({
    ...
    refreshTokens: board.refreshTokens,
  });
  ...
  const [uiSceneId, setUiSceneId] = useState(board.scene?.id);
  if (uiSceneId !== board.scene?.id) {
    setUiSceneId(board.scene?.id);
  ...
  useEffect(() => {
    if (!isDm || board.busy || board.playerPreview || dice.activeRoll || fogOpen) return;
  ...
  if (!board.scene || !board.canvasScene) {
    if (!isDm) {
      return (
        <div className="overflow-hidden rounded-[30px] border border-cyan-400/15 bg-[radial-gradient(circle_at_center,#152338_0%,#0b111b_55%,#070b11_100%)] p-12 text-center">
          ...
          <h2 className="mt-5 text-2xl font-black text-slate-100">{daggerheart ? "The road is lost in the Mists" : "The tabletop is behind the GM screen"}</h2>
    ...
    return <div className="rounded-[30px] border border-slate-800 bg-slate-900/65 p-10 text-center"><h2 className="text-2xl font-black text-slate-100">No VTT scene yet</h2><p className="mt-3 text-sm text-slate-500">Create a prepared scene to initialize the tabletop.</p><button type="button" disabled={board.busy} onClick={() => void board.createScene()} className="mt-4 rounded-xl border border-rose-300/40 px-5 py-3 text-sm text-rose-100">Create a prepared scene</button>{board.error ? <p className="mt-4 text-xs text-rose-300">{board.error}</p> : null}</div>;
  }

  const scene = board.scene;
  const gmBusy = board.busy || Boolean(dice.activeRoll);
  const previewHidden = board.playerPreview && (!scene.is_active || !scene.visible_to_players);
  ...
  return (
    <div ref={boardRef} className={isFullscreen ? "flex h-screen w-screen flex-col gap-2 overflow-hidden bg-[#05080d] p-2 text-slate-100" : "space-y-4"}>
      {board.connection !== "live" && <div role="status" className="flex shrink-0 items-center justify-between gap-3 rounded-xl border border-amber-400/30 bg-amber-950/60 px-4 py-2 text-xs text-amber-100">
        <span>{board.connection === "connecting" ? "Connecting to the table…" : "Connection interrupted. The table may be out of date."}</span>
        <button type="button" onClick={() => void board.refreshFromBroadcast().catch(() => undefined)} className="min-h-9 rounded-lg border border-amber-300/30 px-3 font-bold">Refresh table</button>
      </div>}
      <section className={`flex shrink-0 flex-col gap-3 border border-slate-800 bg-slate-900/70 sm:flex-row sm:items-center sm:justify-between ${isFullscreen ? "rounded-2xl px-4 py-2" : "rounded-[26px] p-4"}`}>
        <div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            {isDm && (!scene.is_active || !scene.visible_to_players) && <span className="text-[10px] text-slate-400">Dice rolls stay behind the GM screen</span>}
          ...
          <p className="mt-1 text-xs text-slate-500">{scene.grid_width} × {scene.grid_height} squares · {daggerheart ? "narrative ranges" : `${scene.feet_per_square} ft`} · {scene.map_original_name ?? "plain grid"}</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          ...
          {isDm ? <button type="button" disabled={Boolean(dice.activeRoll) || board.busy} onClick={() => { setFogOpen(false); fog.cancelDraft(); board.setPlayerPreview(!board.playerPreview); board.selectToken(null, false); }} className={`min-h-9 rounded-xl border px-3 text-[10px] font-black uppercase tracking-[0.12em] ${board.playerPreview ? "border-emerald-300 bg-emerald-300/15 text-emerald-100" : "border-emerald-400/25 bg-emerald-400/5 text-emerald-200"}`}>{board.playerPreview ? "Exit player view" : "Player preview"}</button> : null}
      ...
      <div className={`${isFullscreen ? "grid min-h-0 flex-1 gap-2" : "grid gap-4"} ${isDm && !board.playerPreview ? "xl:grid-cols-[minmax(0,1fr)_360px]" : ""}`}>
        <div className={`relative overflow-hidden border border-slate-800 bg-[#070b11] ${isFullscreen ? "min-h-0 rounded-2xl" : "rounded-[30px]"}`}>
          <div className="absolute left-3 top-3 z-20 flex max-w-[calc(100%-1.5rem)] flex-wrap items-center gap-1.5 rounded-2xl border border-slate-700/80 bg-slate-950/88 p-1.5 shadow-2xl backdrop-blur">
            ...
            {board.measurement ? <span className={`rounded-xl border px-3 py-2 text-[11px] font-black ${board.toolMode === "radius" ? "border-fuchsia-400/30 bg-fuchsia-400/10 text-fuchsia-100" : "border-cyan-400/30 bg-cyan-400/10 text-cyan-100"}`}>{board.toolMode === "radius" ? "Radius " : "Distance "}{!daggerheart && `${board.measurement.feet.toFixed(board.measurement.feet >= 100 ? 0 : 1)} ft · `}{board.measurement.squares.toFixed(1)} sq</span> : null}
          ...
          <VttInitiativeStrip daggerheart={daggerheart} scene={scene} tokens={previewHidden ? [] : fog.displayTokens} />
          {fog.error && <div role="alert" className="absolute right-3 top-24 z-30 max-w-xs rounded-xl border border-rose-400/30 bg-slate-950 p-3 text-xs text-rose-200">{fog.error}</div>}
          ...
          <div className={isFullscreen ? "h-full min-h-0" : "h-[72dvh] min-h-[560px] max-h-[920px]"}>
            {previewHidden ? <div className="flex h-full items-center justify-center bg-[#101018] p-8 text-center"><div><h3 className="font-serif text-2xl font-bold text-rose-100">{daggerheart ? "Behind the Mists" : "Behind the GM screen"}</h3><p className="mt-3 text-sm text-slate-400">Players cannot see this scene. Exit player view to continue preparing it.</p></div></div> : <VttCanvas
          ...
          <VttDiceHistoryPanel open={historyOpen && !previewHidden} isFullscreen={isFullscreen} sceneName={scene.name} isDm={isDm} rolls={dice.historyRolls} loading={dice.historyLoading} clearing={dice.historyClearing} error={dice.historyError} onClose={() => setHistoryOpen(false)} onClear={() => { void dice.clearHistory(); }} />
          {!previewHidden && <VttDiceBar daggerheart={daggerheart} duality={dice.rollKind === "duality"} onDuality={dice.selectDuality}
              onAdversary={isDm && !board.playerPreview ? dice.selectAdversary : undefined} isFullscreen={isFullscreen} counts={dice.counts} modifier={dice.modifier} mode={dice.mode} expression={dice.expression} physicalCount={dice.physicalCount} maxPhysicalDice={dice.maxPhysicalDice} canUseD20Mode={dice.canUseD20Mode} canRoll={dice.canRoll && !previewHidden} activeRoll={dice.activeRoll} latestResult={dice.latestResult} error={dice.error} appearanceName={dice.appearanceName} appearanceSwatch={dice.appearanceSwatch} historyCount={dice.historyRolls.length} historyOpen={historyOpen} onAddDie={dice.addDie} onRemoveDie={dice.removeDie} onModifier={dice.setModifier} onMode={dice.setMode} onClear={dice.clearDice} onHistory={() => setHistoryOpen((current) => !current)} onRoll={() => { void dice.roll(); }} />}
        ...
        {isDm && !board.playerPreview ? (
          <aside className={`${isFullscreen ? "min-h-0 overflow-y-auto rounded-2xl pr-1" : "space-y-4"} space-y-4`}>
            ...
            {daggerheart ? <VttSpotlightPanel scene={scene} tokens={board.allTokens} busy={gmBusy} onSpotlight={(id) => void board.setSpotlight(id)} /> : <VttInitiativePanel scene={scene} tokens={board.allTokens} busy={gmBusy} onInitiative={(tokenId, value) => void board.setTokenInitiative(tokenId, value)} onStart={() => void board.startInitiative()} onPrevious={() => void board.stepInitiative(-1)} onNext={() => void board.stepInitiative(1)} onStop={(clear) => void board.stopInitiative(clear)} />}
