-- Nattau Command: Puzzle Vault v1.1 — Arcane Circuit connectivity hotfix
-- Fixes the Arcane Circuit victory condition.
--
-- Before: every tile had to match one hidden canonical orientation.
-- After: the puzzle is solved when a valid reciprocal conduit path connects
-- the source to every configured destination. Unpowered/disconnected spare
-- tiles no longer prevent a legitimate solution.
...
revoke all on function public.campaign_puzzle_circuit_reaches_targets(integer[],integer[],integer,integer,integer[]) from public;
grant execute on function public.campaign_puzzle_circuit_reaches_targets(integer[],integer[],integer,integer,integer[]) to authenticated;
grant execute on function public.campaign_puzzle_circuit_reaches_targets(integer[],integer[],integer,integer,integer[]) to service_role;

-- Re-check already active Arcane Circuit runs so a board that is currently
-- connected is accepted immediately after this hotfix.
with solved_runs as (
  update public.campaign_puzzle_runs r
  set
    status = 'solved',
    solved_at = coalesce(r.solved_at, now()),
  ...
  where p.id = r.puzzle_id
    and p.puzzle_type = 'arcane_circuit'
    and r.status = 'active'
    and public.campaign_puzzle_circuit_reaches_targets(
      array(
        select value::integer
        from jsonb_array_elements_text(coalesce(p.public_config->'masks','[]'::jsonb))
      ),
      array(
        select value::integer
        from jsonb_array_elements_text(coalesce(r.state->'rotations','[]'::jsonb))
      ),
      greatest(1, coalesce((p.public_config->>'width')::integer, 4)),
      greatest(0, coalesce((p.public_config->>'source_index')::integer, 0)),
      array(
        select value::integer
        from jsonb_array_elements_text(coalesce(p.public_config->'target_indices','[]'::jsonb))
      )
    )
  returning r.puzzle_id
)
update public.campaign_puzzles p
set status = 'solved', updated_at = now()
where p.id in (select puzzle_id from solved_runs);
