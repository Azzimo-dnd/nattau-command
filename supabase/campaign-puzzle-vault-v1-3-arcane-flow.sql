-- Nattau Command: Puzzle Vault v1.3 — Arcane Circuit anchors + live current
--
-- Backend responsibilities:
-- 1. Source/destination tiles are immutable, including against direct RPC calls.
-- 2. Existing Arcane templates are normalized so those anchors begin in the
--    correct solution-facing orientation.
-- 3. Active runs keep all free-tile progress; only their fixed anchors are aligned.
--
-- The animated current itself is client-side and needs no database columns.
