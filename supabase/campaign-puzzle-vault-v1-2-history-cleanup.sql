-- Nattau Command: Puzzle Vault v1.2
-- Solved puzzle history, solver attribution, permanent GM cleanup and RPC hardening.

begin;

alter table public.campaign_puzzle_runs
  add column if not exists solved_by_user_id uuid references auth.users(id) on delete set null,
  add column if not exists solved_by_name text;

-- Backfill existing solved runs using the actor who made the final persisted move.
with final_actor as (
  select distinct on (m.run_id)
    m.run_id,
    m.actor_id,
    coalesce(nullif(trim(p.display_name), ''), 'Solver') as actor_name
  from public.campaign_puzzle_moves m
  left join public.profiles p on p.id = m.actor_id
  order by m.run_id, m.move_number desc
)
update public.campaign_puzzle_runs r
set
  solved_by_user_id = coalesce(r.solved_by_user_id, a.actor_id),
  solved_by_name = coalesce(nullif(trim(r.solved_by_name), ''), a.actor_name)
from final_actor a
where r.id = a.run_id
  and r.status = 'solved'
  and (r.solved_by_user_id is null or nullif(trim(r.solved_by_name), '') is null);

create or replace function public.campaign_puzzle_circuit_reaches_targets(
  p_masks integer[],
  p_rotations integer[],
  p_width integer,
...
begin
  ...
  while v_head <= coalesce(cardinality(v_queue), 0) loop
    ...
    -- North (1) <-> South (4)
    if (v_current_mask & 1) <> 0 and v_row > 0 then
      ...
      if (v_neighbor_mask & 4) <> 0 and not v_visited[v_neighbor + 1] then
    ...
    -- East (2) <-> West (8)
    if (v_current_mask & 2) <> 0 and v_col < p_width - 1 and v_current + 1 < v_count then
      v_neighbor := v_current + 1;
      v_neighbor_mask := public.rotate_campaign_puzzle_mask(
        p_masks[v_neighbor + 1],
        p_rotations[v_neighbor + 1]
      );
      if (v_neighbor_mask & 8) <> 0 and not v_visited[v_neighbor + 1] then
        v_visited[v_neighbor + 1] := true;
        v_queue := array_append(v_queue, v_neighbor);
      end if;
    end if;
    ...
    -- South (4) <-> North (1)
    if (v_current_mask & 4) <> 0 and v_row < v_height - 1 and v_current + p_width < v_count then
      v_neighbor := v_current + p_width;
      v_neighbor_mask := public.rotate_campaign_puzzle_mask(
        p_masks[v_neighbor + 1],
        p_rotations[v_neighbor + 1]
      );
      if (v_neighbor_mask & 1) <> 0 and not v_visited[v_neighbor + 1] then
        v_visited[v_neighbor + 1] := true;
        v_queue := array_append(v_queue, v_neighbor);
      end if;
    end if;
    ...
    -- West (8) <-> East (2)
    if (v_current_mask & 8) <> 0 and v_col > 0 then
      v_neighbor := v_current - 1;
      v_neighbor_mask := public.rotate_campaign_puzzle_mask(
        p_masks[v_neighbor + 1],
        p_rotations[v_neighbor + 1]
      );
      if (v_neighbor_mask & 2) <> 0 and not v_visited[v_neighbor + 1] then
        v_visited[v_neighbor + 1] := true;
        v_queue := array_append(v_queue, v_neighbor);
      end if;
    end if;
...
revoke all on function public.save_campaign_puzzle(text,uuid,jsonb) from public, anon;
revoke all on function public.start_campaign_puzzle(uuid,boolean) from public, anon;
revoke all on function public.set_campaign_puzzle_visibility(uuid,boolean) from public, anon;
revoke all on function public.archive_campaign_puzzle(uuid) from public, anon;
revoke all on function public.delete_campaign_puzzle(uuid) from public, anon;
revoke all on function public.take_campaign_puzzle_control(uuid) from public, anon;
revoke all on function public.heartbeat_campaign_puzzle_control(uuid) from public, anon;
revoke all on function public.release_campaign_puzzle_control(uuid) from public, anon;
revoke all on function public.expire_campaign_puzzle_run(uuid) from public, anon;
revoke all on function public.reveal_campaign_puzzle_sequence(uuid) from public, anon;
revoke all on function public.apply_campaign_puzzle_action(uuid,jsonb) from public, anon;
revoke all on function public.can_access_campaign_puzzle_topic(text) from public, anon;
revoke all on function public.can_manage_campaign_puzzle(uuid) from public, anon;
revoke all on function public.can_view_campaign_puzzle(uuid) from public, anon;
revoke all on function public.build_campaign_puzzle_initial_state(text,jsonb) from public, anon;
revoke all on function public.rotate_campaign_puzzle_mask(integer,integer) from public, anon;
revoke all on function public.campaign_puzzle_circuit_reaches_targets(integer[],integer[],integer,integer,integer[]) from public, anon;

grant execute on function public.save_campaign_puzzle(text,uuid,jsonb) to authenticated, service_role;
grant execute on function public.start_campaign_puzzle(uuid,boolean) to authenticated, service_role;
grant execute on function public.set_campaign_puzzle_visibility(uuid,boolean) to authenticated, service_role;
grant execute on function public.archive_campaign_puzzle(uuid) to authenticated, service_role;
grant execute on function public.delete_campaign_puzzle(uuid) to authenticated, service_role;
grant execute on function public.take_campaign_puzzle_control(uuid) to authenticated, service_role;
grant execute on function public.heartbeat_campaign_puzzle_control(uuid) to authenticated, service_role;
grant execute on function public.release_campaign_puzzle_control(uuid) to authenticated, service_role;
grant execute on function public.expire_campaign_puzzle_run(uuid) to authenticated, service_role;
grant execute on function public.reveal_campaign_puzzle_sequence(uuid) to authenticated, service_role;
grant execute on function public.apply_campaign_puzzle_action(uuid,jsonb) to authenticated, service_role;
grant execute on function public.can_access_campaign_puzzle_topic(text) to authenticated, service_role;
grant execute on function public.can_manage_campaign_puzzle(uuid) to authenticated, service_role;
grant execute on function public.can_view_campaign_puzzle(uuid) to authenticated, service_role;
grant execute on function public.build_campaign_puzzle_initial_state(text,jsonb) to authenticated, service_role;
grant execute on function public.rotate_campaign_puzzle_mask(integer,integer) to authenticated, service_role;
grant execute on function public.campaign_puzzle_circuit_reaches_targets(integer[],integer[],integer,integer,integer[]) to authenticated, service_role;
