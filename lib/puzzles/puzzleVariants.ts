...
// These layouts were generated by legal reverse play from a solved position and
// then verified with the exact one-slide-per-move rules used by the game. The
// minimumMoves value is independently re-checked by the solver below whenever
// a variant is selected, so an accidental edit cannot silently create an
// impossible or mislabelled lock.
...
function legalSlidingStates(blocks: SlidingBlock[]) {
  ...
  for (let index = 0; index < blocks.length; index += 1) {
    ...
    const directions = block.axis === "h" ? [[-1, 0], [1, 0]] : [[0, -1], [0, 1]];
...
function materializeVariant(variant: SlidingVariant) {
  const positions = new Map(variant.positions.map(([id, x, y]) => [id, { x, y }]));
